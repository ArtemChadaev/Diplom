...
- **СТРОГИЙ ЗАПРЕТ**: Запрещено использовать пакет `fmt` (`fmt.Println`, `fmt.Printf` и т.д.) для вывода логов или отладочной информации.
- Вместо стандартных пакетов `log/slog` или `fmt`, всегда используйте только наш логгер.
