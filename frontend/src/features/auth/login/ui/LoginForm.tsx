...
export function LoginForm({ onFound, onNotFound, defaultEmail = "", externalError = null }: LoginFormProps) {
  ...
  const handleGoogleLogin = () => {
    const clientId = (import.meta.env.VITE_GOOGLE_CLIENT_ID as string | undefined) ?? "691345228451-172nt62vjc3pirkbumpjqcr6pd280pbp.apps.googleusercontent.com"
    ...
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scope)}&response_type=${encodeURIComponent(responseType)}&nonce=${encodeURIComponent(nonce)}`
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
      <form onSubmit={onSubmit}>
        ...
        {(form.formState.errors.email ?? error ?? externalError) && (
      ...
      >
        ...
        Войти через
        <svg className="w-5 h-5 ml-2 inline-block transition-transform group-hover:scale-110 align-middle" viewBox="0 0 24 24">
